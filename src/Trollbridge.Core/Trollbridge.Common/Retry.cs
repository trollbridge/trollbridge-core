    ...
    public static class Retry
    {
        ...
        public static RetryPolicy CurrentRetryPolicy
        {
            get
            {
                ...
                {
                    string temp = Utilities.GetAppSettingsStringValue("RetryCount");
                    int RetryCount = Convert.ToInt32(temp.Length == 0 ? "5" : temp);

                    temp = Utilities.GetAppSettingsStringValue("RetryMinimunDelay");
                    string RetryMinimunDelay = temp.Length == 0 ? "00:00:00.1" : temp;

                    temp = Utilities.GetAppSettingsStringValue("RetryMaximunDelay");
                    string RetryMaximunDelay = temp.Length == 0 ? "00:00:05" : temp;

                    temp = Utilities.GetAppSettingsStringValue("RetryInitialDelay");
        ...
        public static bool IsTransient(Exception ex)
        {
            if (ex != null)
            {
                ...
                {
                    string temp = Utilities.GetAppSettingsStringValue("TransientErrorCodesRetry");
                    _sqlErrorCodes = temp.Split('|');
                    if (_sqlErrorCodes == null || _sqlErrorCodes.Length < 1)
                    {
                        _sqlErrorCodes = "-2|64|233|08001|08S01|10053|10054|10060|40001|40143|40174|40197|40501|40544|40549|40550|40551|40552|40553|40613|40615".Split('|');
                    }
                }
